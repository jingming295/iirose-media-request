...
export class MediaParsing
{
    ...
    {
        let checkCpuUsageIntervalId: NodeJS.Timeout | undefined = undefined;
        ...
        try
        {
            ...
            checkCpuUsageIntervalId = setInterval(async () =>
            {
                osUtils.cpuUsage((v) =>
                {
                    if (v > maxCpuUsage)
    ...
    private async getMedia(page: Page, originUrl: string, timeOut: number, waitTime: number, ctx: Context, client: CDPSession): Promise<MediaData[]>
    {
        ...
        /**
         * 从网站meta tag 中 提取出 keyword 和 Open Graph (如果有)
         * @param originUrl 网址
         * @returns og, keyword
         */
     ...
     * @returns 
     */
    private async processMediaData(resourceUrls: ResourceUrls[], mediaType: ('music' | 'video')[], cover: string[], name: string[], signer: string[], ctx: Context): Promise<MediaData[]>
    {
        let url: string[] = [];
